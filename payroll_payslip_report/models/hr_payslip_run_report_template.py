...
class HrPayslipRunReportTemplate(models.Model):
    _name = "hr.payslip.run.report.template"
    _description = "Payslip Batch Conference Template"
    _order = "name"
    ...
    report_template_id = fields.Many2one(
        ...
        domain=[("type", "=", "qweb"), ("key", "like", "payroll_payslip_report.%")],
        required=True,
        ondelete="restrict",
    )
    ...
    def action_duplicate(self):
        ...
        new_view = original_view.copy(
            {
                "name": new_view_name,
                "key": new_view_key,
                "model": original_view.model or "hr.payslip.run",
            }
        )
        ...
        return {
            ...
            "name": _("Payslip Batch Conference Template"),
            "res_model": "hr.payslip.run.report.template",
